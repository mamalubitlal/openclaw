...
function createPluginSdkAliasMapForSourceTests(): Record<string, string> {
  const aliasMap: Record<string, string> = {
    "openclaw/plugin-sdk": path.join(process.cwd(), "src", "plugin-sdk", "root-alias.cjs"),
  };
  for (const subpath of __testing.listPluginSdkExportedSubpaths()) {
    const srcPath = path.join(process.cwd(), "src", "plugin-sdk", `${subpath}.ts`);
    if (fs.existsSync(srcPath)) {
      aliasMap[`openclaw/plugin-sdk/${subpath}`] = srcPath;
    }
  }
  return aliasMap;
}
...
module.exports = {
  ...
  it("loads source runtime shims through the non-native Jiti boundary", async () => {
    const jiti = createJiti(import.meta.url, {
      ...__testing.buildPluginLoaderJitiOptions(createPluginSdkAliasMapForSourceTests()),
      tryNative: false,
    });
